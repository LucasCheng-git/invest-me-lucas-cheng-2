...
export default function InvestMe() {
  ...
  return (
    <div style={S.app}>
      ...
      {["feed","profile","messages","chat","watch","community"].includes(screen) && me && (
      ...
      <div style={{...S.aCard,maxWidth:440}}>
        ...
        {step===1&&<>
          ...
          <div style={{display:"flex",gap:12,margin:"8px 0 16px"}}>
            {["startup","investor"].map(r=>(<div key={r} style={{...S.rCard,...(role===r?S.rCardOn:{})}} onClick={()=>setRole(r)}><span style={{fontSize:30}}>{r==="startup"?"🚀":"💼"}</span><span style={{fontWeight:700,marginTop:6,fontSize:14}}>{r==="startup"?"Startup":"Investor"}</span><span style={{color:"#bbb",fontSize:11,textAlign:"center"}}>{r==="startup"?"Pitch your idea":"Discover deals"}</span></div>))}
          </div>
        ...
        {step===2&&<>
          ...
          <div style={{position:"relative"}}>
            <textarea style={{...S.inp,height:84,resize:"none",paddingBottom:26,borderColor:!wcOk?"#ff3b30":undefined}} placeholder="Bio (max 250 words)" value={form.bio} onChange={e=>{if(countWords(e.target.value)<=255)sf("bio",e.target.value);}}/>
            <span style={{position:"absolute",bottom:9,right:12,fontSize:11,color:!wcOk?"#ff3b30":wc>220?"#ff9500":"#bbb"}}>{wc}/250</span>
          </div>
          ...
          <div style={{display:"flex",gap:10}}>
            ...
            <button style={{...S.btnP,flex:2}} onClick={()=>{if(!form.name||!form.email||!form.password){notify("Fill required fields","error");return;}if(!wcOk){notify("Bio too long","error");return;}setStep(3);}}>Next</button>
          </div>
        </>}
        {step===3&&role==="startup"&&<>
          <h2 style={S.aTitle}>Your industry</h2>
          <div style={S.indGrid}>{INDUSTRIES.map(ind=>(<div key={ind} style={{...S.iChip,...(form.industry===ind?S.iChipOn:{})}} onClick={()=>sf("industry",ind)}>{ICONS[ind]} {ind}</div>))}</div>
        ...
        {step===3&&role==="investor"&&<>
          ...
          <div style={S.indGrid}>{INDUSTRIES.map(ind=>(<div key={ind} style={{...S.iChip,...(form.industries.includes(ind)?S.iChipOn:{})}} onClick={()=>togInd(ind)}>{ICONS[ind]} {ind}</div>))}</div>
          <div style={{display:"flex",gap:10,marginTop:10}}>
            <button style={{...S.btnG,flex:1}} onClick={()=>setStep(2)}>Back</button>
            <button style={{...S.btnP,flex:2,opacity:form.industries.length?1:0.4}} disabled={!form.industries.length||loading} onClick={submit}>{loading?"Creating…":"Create Account 🎉"}</button>
          </div>
        </>}
        {step===4&&role==="startup"&&<>
          <h2 style={S.aTitle}>Pitch video</h2>
          <input style={{...S.inp,marginBottom:2}} placeholder="Pitch title" value={form.pitchTitle} onChange={e=>sf("pitchTitle",e.target.value)}/>
          <p style={{color:"#bbb",fontSize:11,margin:"0 0 14px",textAlign:"center"}}>⏱ Max 3 minutes</p>
          {!videoMode&&!videoBlob&&(<div style={{display:"flex",gap:12}}><div style={S.vmCard} className="vmcard" onClick={()=>setVideoMode("record")}><span style={{fontSize:32}}>📹</span><span style={{fontWeight:700,fontSize:13,marginTop:8}}>Record</span></div><div style={S.vmCard} className="vmcard" onClick={()=>setVideoMode("upload")}><span style={{fontSize:32}}>📁</span><span style={{fontWeight:700,fontSize:13,marginTop:8}}>Upload</span></div></div>)}
          ...
          <div style={{display:"flex",gap:10,marginTop:10}}>
            ...
            <button style={{...S.btnP,flex:2,opacity:loading?0.7:1}} disabled={loading} onClick={submit}>{loading?"Uploading…":videoBlob?"Launch 🚀":"Skip →"}</button>
          </div>
        </>}
      </div>
    </div>
...
function Shell({me,setMe,screen,setScreen,logout,watchTarget,setWatchTarget,chatTarget,setChatTarget,filterInd,setFilterInd,notify}) {
  ...
  const navItems=[{icon:"🏠",lbl:"Feed",s:"feed"},{icon:"🌐",lbl:"Community",s:"community"},{icon:"💬",lbl:"DMs",s:"messages",badge:unread},{icon:"👤",lbl:"Profile",s:"profile"}];

  return (
    <div style={{display:"flex",flexDirection:"column",minHeight:"100vh"}}>
      <header style={S.hdr}>
        <div style={S.logoRow} onClick={()=>setScreen("feed")} className="cp"><div style={S.lm}>I</div><span style={S.lt}>InvestMe</span></div>
        <nav style={{display:"flex",gap:2}}>
          {navItems.map(n=>(<button key={n.s} style={{...S.nBtn,...((screen===n.s||(screen==="chat"&&n.s==="messages")||(screen==="watch"&&n.s==="feed"))?S.nBtnOn:{})}} onClick={()=>setScreen(n.s)}><span style={{position:"relative",fontSize:20}}>{n.icon}{n.badge>0&&<span style={S.badge}>{n.badge}</span>}</span><span style={{fontSize:9,color:(screen===n.s)?"#1a6cf5":"#aaa"}}>{n.lbl}</span></button>))}
          <button style={S.nBtn} onClick={logout}><span style={{fontSize:20}}>↩️</span><span style={{fontSize:9,color:"#aaa"}}>Out</span></button>
        </nav>
      </header>
...
function Feed({me,displayS,filterInd,setFilterInd,setWatchTarget,setScreen,setChatTarget,notify}) {
  ...
  async function toggleLike(sid){
    const isLiked=likedMap[sid];
    setLikedMap(m=>({...m,[sid]:!isLiked}));setLikeCounts(c=>({...c,[sid]:(c[sid]||0)+(isLiked?-1:1)}));
    if(isLiked)await sb.from("likes").delete().eq("user_id",me.id).eq("startup_id",sid);
    else await sb.from("likes").insert({user_id:me.id,startup_id:sid});
  }

  async function toggleFollow(uid){
    const isFollowing=followMap[uid];
    setFollowMap(m=>({...m,[uid]:!isFollowing}));
    if(isFollowing)await sb.from("follows").delete().eq("follower_id",me.id).eq("following_id",uid);
    else{await sb.from("follows").insert({follower_id:me.id,following_id:uid});notify("Following! 🎉","success");}
  }
    ...
    <div style={{paddingBottom:40}}>
      ...
      <div style={{padding:"12px 14px 4px"}}>
        <div style={{position:"relative"}}>
          ...
          <input style={{...S.inp,paddingLeft:38,borderRadius:24,background:"#f5f7fa",border:"1.5px solid #eee"}} placeholder="Search pitches, industries…" value={search} onChange={e=>setSearch(e.target.value)}/>
          {search&&<button style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",fontSize:15,color:"#aaa"}} onClick={()=>setSearch("")}>✕</button>}
        </div>
      </div>
      {/* Filters */}
      ...
      <div style={S.card}>
        ...
        <div style={{padding:"16px 20px 20px"}}>
          <div style={{display:"flex",alignItems:"center",marginBottom:12}}>
            <Av src={startup.avatar_url} name={startup.name} size={50}/>
            <div style={{marginLeft:12}}><div style={{fontWeight:700,fontSize:17}}>{startup.name}</div><div style={{color:"#1a6cf5",fontSize:13}}>{ICONS[startup.industry]} {startup.industry}</div></div>
          </div>
...
// ── PROFILE with avatar upload + edit ─────────────────────────────────────────
      ...
      <div style={{background:"#fff",borderRadius:18,padding:20,boxShadow:"0 2px 12px rgba(0,0,0,0.05)",marginBottom:20}}>
        <div style={{display:"flex",alignItems:"flex-start",marginBottom:16}}>
          ...
          <div style={{position:"relative",flexShrink:0}}>
            ...
            <button onClick={()=>avatarRef.current.click()} title="Change photo"
              style={{position:"absolute",bottom:0,right:0,background:"#1a6cf5",border:"2.5px solid #fff",borderRadius:"50%",width:28,height:28,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:12,padding:0}}>
          ...
          <div style={{marginLeft:14,flex:1}}>
            {!editing?<>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                ...
                {isS&&me.industry&&<span style={{background:"#f0f5ff",color:"#1a6cf5",borderRadius:20,padding:"4px 12px",fontSize:12,fontWeight:600}}>{ICONS[me.industry]} {me.industry}</span>}
              </div>
            </>:<>
        ...
        {editing&&<>
          <div style={{position:"relative",marginBottom:8}}>
            <textarea style={{...S.inp,height:90,resize:"none",paddingBottom:24,borderColor:!wcOk?"#ff3b30":undefined}} placeholder="Bio (max 250 words)" value={editBio} onChange={e=>{if(countWords(e.target.value)<=255)setEditBio(e.target.value);}}/>
            <span style={{position:"absolute",bottom:9,right:12,fontSize:11,color:!wcOk?"#ff3b30":wc>220?"#ff9500":"#bbb"}}>{wc}/250</span>
          </div>
          {isS&&<>
            <p style={{margin:"4px 0 8px",fontWeight:600,fontSize:13}}>Industry</p>
            <div style={{...S.indGrid,maxHeight:160,marginBottom:12}}>{INDUSTRIES.map(ind=>(<div key={ind} style={{...S.iChip,...(editIndustry===ind?S.iChipOn:{})}} onClick={()=>setEditIndustry(ind)}>{ICONS[ind]} {ind}</div>))}</div>
          </>}
          {!isS&&<>
            <p style={{margin:"4px 0 8px",fontWeight:600,fontSize:13}}>Industries you invest in</p>
            <div style={{...S.indGrid,maxHeight:160,marginBottom:12}}>{INDUSTRIES.map(ind=>(<div key={ind} style={{...S.iChip,...(editIndustries.includes(ind)?S.iChipOn:{})}} onClick={()=>togInd(ind)}>{ICONS[ind]} {ind}</div>))}</div>
          </>}
          <div style={{display:"flex",gap:10}}>
            <button style={{...S.btnG,flex:1}} onClick={()=>{setEditing(false);setEditName(me.name);setEditBio(me.bio||"");setEditIndustry(me.industry||"");setEditIndustries(me.industries||[]);}}>Cancel</button>
            <button style={{...S.btnP,flex:2,opacity:saving?0.7:1}} disabled={saving} onClick={saveProfile}>{saving?"Saving…":"Save Changes ✅"}</button>
          </div>
        </>}
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 62px)"}}>
      <div style={{display:"flex",alignItems:"center",padding:"12px 16px",borderBottom:"1px solid #f0f0f0",background:"#fff"}}>
        ...
        <Av src={peer.avatar_url} name={peer.name} size={38}/>
        <div style={{marginLeft:10}}><div style={{fontWeight:700}}>{peer.name}</div><div style={{fontSize:12,color:"#aaa"}}>{peer.role==="startup"?peer.industry:peer.firm}</div></div>
      </div>
...
function Community({me,allUsers,notify}) {
  ...
  async function loadPosts(){
    setLoading(true);
    const {data}=await sb.from("posts").select("*").eq("community",tab).order("created_at",{ascending:false});
    const ps=data||[];setPosts(ps);
    if(ps.length){
      const ids=ps.map(p=>p.id);
      sb.from("post_upvotes").select("post_id").in("post_id",ids).then(({data:uv})=>{const c={};(uv||[]).forEach(u=>{c[u.post_id]=(c[u.post_id]||0)+1;});setUpvoteCounts(c);});
      sb.from("post_upvotes").select("post_id").eq("user_id",me.id).in("post_id",ids).then(({data:uv})=>{const m={};(uv||[]).forEach(u=>{m[u.post_id]=true;});setUpvotedMap(m);});
    }
    setLoading(false);
  }

  async function submitPost(){
    if(!newTitle.trim()||!newBody.trim()){notify("Fill in title and body","error");return;}
    setPosting(true);
    const {error}=await sb.from("posts").insert({author_id:me.id,community:tab,title:newTitle,body:newBody});
    if(error)notify("❌ Post failed: "+error.message,"error");
    else{notify("Posted! 🎉","success");setNewTitle("");setNewBody("");setShowNew(false);loadPosts();}
    setPosting(false);
  }

  async function toggleUpvote(postId){
    const isUp=upvotedMap[postId];
    setUpvotedMap(m=>({...m,[postId]:!isUp}));setUpvoteCounts(c=>({...c,[postId]:(c[postId]||0)+(isUp?-1:1)}));
    if(isUp)await sb.from("post_upvotes").delete().eq("user_id",me.id).eq("post_id",postId);
    else await sb.from("post_upvotes").insert({user_id:me.id,post_id:postId});
  }
  ...
  return (
    <div style={{paddingBottom:80}}>
      <div style={{background:"linear-gradient(135deg,#1a6cf5,#0d4fc4)",padding:"20px 16px 16px",color:"#fff"}}>
        <h2 style={{margin:"0 0 4px",fontSize:20,fontWeight:800}}>🌐 Community</h2>
        <p style={{margin:0,fontSize:13,opacity:0.8}}>Connect with your peers</p>
      </div>
      <div style={{display:"flex",borderBottom:"2px solid #f0f0f0",background:"#fff"}}>
        {["investors","startups"].map(t=>(<button key={t} style={{flex:1,padding:"14px 0",border:"none",background:"none",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:14,color:tab===t?"#1a6cf5":"#aaa",borderBottom:tab===t?"3px solid #1a6cf5":"3px solid transparent"}} onClick={()=>setTab(t)}>{t==="investors"?"💼 Investors":"🚀 Startups"}</button>))}
      </div>
      <div style={{padding:"16px 14px"}}>
        <div style={{background:"#f0f5ff",borderRadius:14,padding:"12px 14px",marginBottom:16,display:"flex",alignItems:"center",gap:10}}>
          <span style={{fontSize:28}}>{tab==="investors"?"💼":"🚀"}</span>
          <div><div style={{fontWeight:700,fontSize:14}}>{tab==="investors"?"Investor Hub":"Founder Space"}</div><div style={{color:"#666",fontSize:12}}>{tab==="investors"?"Share insights, ask questions, connect":"Share learnings, find co-founders, get support"}</div></div>
        </div>
        ...
        {showNew&&(
          <div style={{background:"#fff",borderRadius:16,padding:16,boxShadow:"0 2px 14px rgba(0,0,0,0.06)",marginBottom:16}}>
            <h3 style={{margin:"0 0 12px",fontSize:16,fontWeight:800}}>New Post</h3>
            <input style={{...S.inp,marginBottom:10}} placeholder="Title *" value={newTitle} onChange={e=>setNewTitle(e.target.value)}/>
            <textarea style={{...S.inp,height:100,resize:"none",marginBottom:10}} placeholder="Share your thoughts…" value={newBody} onChange={e=>setNewBody(e.target.value)}/>
            <div style={{display:"flex",gap:10}}>
              <button style={{...S.btnG,flex:1}} onClick={()=>{setShowNew(false);setNewTitle("");setNewBody("");}}>Cancel</button>
              <button style={{...S.btnP,flex:2,opacity:posting?0.7:1}} disabled={posting} onClick={submitPost}>{posting?"Posting…":"Post 🚀"}</button>
            </div>
          </div>
        )}
        {loading&&<Spinner/>}
        ...
        {!loading&&posts.map(post=>{
          ...
          return (
            <div key={post.id} style={{background:"#fff",borderRadius:16,boxShadow:"0 2px 10px rgba(0,0,0,0.05)",marginBottom:12,overflow:"hidden"}}>
              <div style={{padding:"14px 16px",cursor:"pointer"}} onClick={()=>setOpenPost(post)}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                  <Av src={author?.avatar_url} name={author?.name||"?"} size={32}/>
                  <div><div style={{fontWeight:600,fontSize:13}}>{author?.name||"Unknown"}</div><div style={{color:"#bbb",fontSize:11}}>{timeAgo(post.created_at)}</div></div>
                </div>
                <h3 style={{margin:"0 0 6px",fontSize:15,fontWeight:700}}>{post.title}</h3>
                <p style={{margin:0,color:"#666",fontSize:13,lineHeight:1.5,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>{post.body}</p>
              </div>
              <div style={{display:"flex",gap:8,padding:"8px 16px 12px",borderTop:"1px solid #f5f5f5"}}>
                <button style={{display:"flex",alignItems:"center",gap:5,background:upvotedMap[post.id]?"#f0f5ff":"#f5f7fa",border:upvotedMap[post.id]?"1.5px solid #1a6cf5":"1.5px solid #eee",borderRadius:20,padding:"5px 12px",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:13,color:upvotedMap[post.id]?"#1a6cf5":"#666"}} onClick={()=>toggleUpvote(post.id)}>▲ {upvoteCounts[post.id]||0}</button>
                <button style={{display:"flex",alignItems:"center",gap:5,background:"#f5f7fa",border:"1.5px solid #eee",borderRadius:20,padding:"5px 12px",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontSize:13,color:"#666"}} onClick={()=>setOpenPost(post)}>💬 Comments</button>
              </div>
            </div>
          );
        })}
...
function PostDetail({post,me,allUsers,onBack,notify,upvoted,upvoteCount,toggleUpvote}) {
  ...
  async function submitComment(){
    ...
    const {data,error}=await sb.from("comments").insert({post_id:post.id,author_id:me.id,body:newComment}).select().single();
    if(error)notify("❌ Comment failed","error");else{setComments(p=>[...p,data]);setNewComment("");}
    setPosting(false);
  }

  return (
    <div style={{paddingBottom:80}}>
      <div style={{background:"#fff",borderBottom:"1px solid #f0f0f0",padding:"12px 16px",display:"flex",alignItems:"center",gap:8,position:"sticky",top:0,zIndex:10}}>
        <button style={{...S.back,padding:0,marginRight:4}} onClick={onBack}>←</button>
        <span style={{fontWeight:700,fontSize:15}}>Post</span>
      </div>
      <div style={{padding:"16px 14px"}}>
        <div style={{background:"#fff",borderRadius:16,padding:16,boxShadow:"0 2px 10px rgba(0,0,0,0.05)",marginBottom:16}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}><Av src={author?.avatar_url} name={author?.name||"?"} size={36}/><div><div style={{fontWeight:700}}>{author?.name}</div><div style={{color:"#bbb",fontSize:11}}>{timeAgo(post.created_at)}</div></div></div>
          <h2 style={{margin:"0 0 10px",fontSize:18,fontWeight:800,lineHeight:1.3}}>{post.title}</h2>
          <p style={{margin:"0 0 14px",color:"#555",lineHeight:1.65,fontSize:14}}>{post.body}</p>
          <button style={{display:"flex",alignItems:"center",gap:6,background:upvoted?"#f0f5ff":"#f5f7fa",border:upvoted?"1.5px solid #1a6cf5":"1.5px solid #eee",borderRadius:20,padding:"6px 14px",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:14,color:upvoted?"#1a6cf5":"#666"}} onClick={()=>toggleUpvote(post.id)}>▲ {upvoteCount} Upvotes</button>
        </div>
        ...
        <div style={{display:"flex",gap:10,marginTop:8}}>
          <Av src={me.avatar_url} name={me.name} size={36}/>
          <div style={{flex:1,display:"flex",gap:8}}>
            <input style={{...S.inp,borderRadius:24,flex:1}} placeholder="Add a comment…" value={newComment} onChange={e=>setNewComment(e.target.value)} onKeyDown={e=>e.key==="Enter"&&submitComment()}/>
            <button style={{background:posting?"#aaa":"#1a6cf5",color:"#fff",border:"none",borderRadius:24,padding:"0 16px",fontSize:16,cursor:"pointer"}} onClick={submitComment}>➤</button>
          </div>
        </div>
      </div>
    </div>
...
const S={
  ...
  nBtn:{background:"none",border:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",padding:"6px 8px",borderRadius:10,gap:2,position:"relative"},
